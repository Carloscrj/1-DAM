...
import pruebas.model.PersonaHS;

public class PruebaHashSetObj {
	static HashSet<PersonaHS> setPersonas;
	static Scanner sc;
	
	public static void main(String[] args) {
		sc= new Scanner(System.in);
		
		setPersonas= new HashSet<PersonaHS>();
	...
	private static void rellenarSet() {
		...
		while(continuar) {  //aquí utuliza este metodo para parar(no para que siga como en los arrays).
				...
				persona = new PersonaHS(nombre, edad);
	...
	private static void mostrarSet() {
		...
		if(setPersonas.isEmpty()) {
			...
			for (PersonaHS persona : setPersonas) { //la primera parte es cada elemento, la segunda es la colección
