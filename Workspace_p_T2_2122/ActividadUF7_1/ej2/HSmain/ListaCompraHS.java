...
import Productoej2HS.pojo.ProductoHS;
...
System.out.println("\n Elementos de la lista");
	...
	private static void añadirProducto() {
		...
		int cantidad;
			...
			ProductoHS producto = new ProductoHS(nombre, cantidad);
	...
	private static void eliminarProducto() {
		...
		/*Object[] listaCompra1= listaCompra.toArray();
		ArrayList<Object> listaCompra1 = new ArrayList<>(listaCompra);*/
		ArrayList<ProductoHS>listaCompra1 = new ArrayList<>(listaCompra);
	
		listaCompra.remove(listaCompra1)
		if(listaCompra1.isEmpty()) {
		...
		for (int i = 0; i < listaCompra.size(); i++) {
			...
			if(listaCompra1.get(i).getNombre().equalsIgnoreCase(elimpro)) {
				System.out.println("El producto ha sido eliminado."); //hay que poner primero el syso, porque si lo eliminamos 
				listaCompra1.remove(i);								  //primero no hace bien el bucle, ya que no lo encuentra
			}else {
